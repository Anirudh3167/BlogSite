...
import { getUser, getUserBlogs, updateBlogStats } from '@/apiFunctions';
...
export default function Feed() {
    ...
    useEffect(() => {
        const userBlogs = async (username) => {
            setUName(username);
            // const res = await axios.get(backend_url + '/user-blogs',{params:{'username': username}, withCredentials: true});
            const res = await getUserBlogs(username);
            if (!res.status) {alert('Invalid'); return ;}
            setBlogs(JSON.parse(res.author_blogs));
            console.log(JSON.parse(res.author_blogs));
        }
        const isAuthenticated = async () => {
            ...
            setLoading(false);
            if (!res.status) router.push('/signin');
            else userBlogs(res.user.username);
        }
        isAuthenticated();
      }, []);
      ...
      const deleteBlogAction = async (blog_id) => {
        ...
        if (res.status) setBlogs((blog) => blog.filter((b) => b.id !== blog_id));
      }
      const updateAction = async (action, id) => {
        ...
        const res = await updateBlogStats({action,blog_id:id});
        console.log(res);
            ...
            { blogs.map((blog,index) => {
                return(
                    <div className={Styles.blogContainer} key={index}>
                        ...
                        <div className={Styles.deleteBlogBtn} onClick={()=>{deleteBlogAction(blog.id)}}> Delete </div>
                        </div>
